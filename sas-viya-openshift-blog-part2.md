...
|  |`sas-programming-environment` / `hostmount-anyuid`** |**Optional**|Only needed if SAS compute session needs to access data on hostPath mounts|
...
| **OPENSEARCH** |`sas-opendistro`|**Optional** \*** |For optimal performance, deploying OpenSearch software requires changes to a kernel setting. Optionally, you can use a MachineConfig to apply the kernel parameter as documented below.|


<img width="1089" alt="image" src="sas-service-scc-table.png">
